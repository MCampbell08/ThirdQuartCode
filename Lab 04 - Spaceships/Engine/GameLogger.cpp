...
bool GameLogger::Initialize(const char * const path, const char * const filename)
{
	...
	m_logStream << t << "[" << std::setw(15) << std::left <<  GetMsg(LogMsgType::Info) << "]" << "GameLogger::Initialize() opened log file [" << filepath << "] for writing.\n";
	if (m_logStream)
	{
		WriteLog(GetMsg(LogMsgType::Process), "GameLogger::Initialize() successful\n");
		return true;
	}
	else
	{
...
bool GameLogger::Shutdown()
{
	if (m_logStream)
	{
		WriteLog(GetMsg(LogMsgType::Process), "GameLogger::Shutdown() successful\n");
		m_logStream.close();
		ShowStats(std::cout);
		return true;
	}
...
void GameLogger::WriteLog(LogMsgType msgType, const char * const msg)
{
	...
	if (m_logStream)
	{
		m_logStream << std::setw(3) << t << "[" << std::setw(15) << std::left << msgType << "]" << msg;
	}
	else {
		msgType = LogMsgType::FatalError;
		m_logStream << std::setw(3) << t << "[" << std::setw(15) << std::left << msgType << "]" << "Unable to write to file!" << msg;
	}
	m_logStream.flush();
}
